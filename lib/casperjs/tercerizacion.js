...
var casper = require('casper').create({
	...
	stepTimeout: 30000, // 30 seconds timeout for each step
...
casper.start(WEB_ADDRESS, function() {
  this.emit(ECHO_EVENT_NAME, 'Fomulario de login cargado correctamente');
...
casper.waitForSelector('iframe#ifrVCE', function() {
  ...
  this.emit(ECHO_EVENT_NAME, 'Contribuyente logeado correctamente');
...
casper.waitForSelector('iframe#iframeApplication', function() {
	this.withFrame('iframeApplication', function() {
        ...
        var PSENameElement,
          ...
          PSENameElement = (
            elements[i].querySelector("td[idx='2']").innerText//.toString().trim()
          );

          PSEIdentityElement = (
            elements[i].querySelector("td[idx='0']").innerText//.toString().trim()
          );

          PSEAuthorizationElement = (
            elements[i].querySelector("td[idx='3']").innerText//.toString().trim()
          );

          // NOTE: Verificamos si el RUC del PSE que intentamos registrar,
          // es igual al RUC del PSE que se encuentra registrado
  				if ((String(PSEIdentityElement).trim() === String(argPseIdentity).trim()) && String(PSEIdentityElement).trim() != '' && String(argPseIdentity).trim() != '') {
            return {
              name: PSENameElement,
      ...
      return (
        Object.keys(PSERegistrationInformation || {}).length > 0 ? true : false
      );
		}, function then() {// step to execute when check() is ok

			this.emit(CAPTURE_EVENT_NAME, 'ExisteAltaAutorizacion');

			this.emit(INFO_ERROR_EVENT_NAME, 'Inicio de autorización ' + PSEInformationAlreadyRegistered.authorization);
		}, function timeout() {
      this.emit(CAPTURE_EVENT_NAME, 'NoExisteAltaAutorizacion');
      ...
      this.emit(CAPTURE_EVENT_NAME, 'FormularioBusquedaPSEIncorrecto');
        ...
        // NOTE: Botón para registrar el alta
			  document.getElementById('btnNuevoCert1').click();
	    }, AUTHORIZATION_DATE);
  ...
  // NOTE: Verificando si el contribuyente tiene permisos al menú "Altas/Bajas de PSE"
  if (!this.exists('#nivel4_11_9_6_1_1')) {
    this.emit(CAPTURE_EVENT_NAME, 'SinPermisosAltas');

    this.emit(DANGER_ERROR_EVENT_NAME, 'Sin permisos para registar una alta de autorización');
  } else {
