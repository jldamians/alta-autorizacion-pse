...
var IDENTITY = String(browser.cli.get("identity")),
    USERNAME = String(browser.cli.get("username")),
    PASSWORD = String(browser.cli.get("password")),
    PERIODO_TRIBUTARIO = String(browser.cli.get("period")),
    VENTAS_GRAVADAS_NETAS = String(browser.cli.get("sales")),
    COMPRAS_GRAVADAS_NETAS = String(browser.cli.get("purchases")),
    INGRESOS_NETOS = String(browser.cli.get("income")),
    IMPORTE_PAGAR = browser.cli.get("payment"),
    RUTA_CONSTANCIA = String(browser.cli.get("path"));
...
browser.on('remote.alert', function(message) {
  var exists = "Existe una declaración presentada para el mismo periodo, esta declaración  será considerada rectificatoria.",
      renta = "SUNAT generará una Orden de Pago por la diferencia en Renta de 3ra..";

  if (message === exists) {
    this.emit(INFO_EVENT_NAME, "EXISTE UNA DECLARACIÓN PRESENTADA", true);
  } else if (message === renta) {
    this.emit(INFO_EVENT_NAME, "SUNAT GENERARÁ ORDEN DE PAGO");
  } else {
    this.emit(ERROR_EVENT_NAME, message, true);
  }

  this.emit(CAPTURE_EVENT_NAME, 'REMOTE-ALERT');
});
